...
def select_3():
    ...
    result = session.query(Subject.name, func.round(func.avg(Grade.grade), 2).label('average_grade')) \
        .select_from(Grade).join(Subject).filter(Grade.subjects_id == 50).group_by(Subject.id).limit(1).all()
    return result
...
def select_5():
    ...
    result = session.query(Teacher.fullname, Subject.name).select_from(Subject)\
        .join(Teacher).filter(and_(Subject.teacher_id == Teacher.id, Teacher.id == 11)).all()
    return result
...
def select_6():
    ...
    result = session.query(Student.id, Student.fullname, Group.name)\
        ...
        .filter(and_(Student.group_id == Group.id, Group.id == 8)).all()
    return result
...
join (select * from grades g2 join subjects s2 on g2.subject_id = s2.id where s2.id = 50) as g on g.student_id = s1.id;
    """
    stmt = exists().where(and_(Student.group_id == Group.id, Group.id == 8))
    result = session.query(Student.fullname, Grade.grade, Subject.name).select_from(Grade)\
              .join(Subject).filter(and_(Grade.subjects_id == Subject.id, Subject.id == 50, stmt)).all()
    return result
...
def select_8():
    ...
    """
    stmt = exists().where(and_(Subject.teacher_id == Teacher.id, Teacher.id == 12))
    result =session.query(func.round(func.avg(Grade.grade), 2).label('average_grade'))\
...
def select_9():
    ...
    """
    stmt = exists().where(and_(Grade.student_id == Student.id, Student.id == 101))
...
def select_10():
    ...
    stmt = exists().where(and_(Subject.teacher_id == Teacher.id, Teacher.id == 12))
    result = session.query(Subject.name.distinct()).select_from(Grade)\
        .join(Student).filter(and_(Grade.student_id == Student.id, Student.id == 100, stmt)).all()
    return result


def select_11():
    stmt = exists().where(and_(Subject.teacher_id == Teacher.id, Teacher.id == 11))
...
def select_12():
    ...
    where g.subject_id =46 and s.group_id = 9 and g.date_of = (
    ...
    subquery = (select(func.max(Grade.date_of)).join(Student)\
                .filter(and_(Grade.subjects_id == 46, Student.group_id == 9))).scalar_subquery()
